...
| `Desktop Shortcuts\` | Desktop originals DeskRipple has tucked away for you. By default, dragging a `.lnk`/`.url` shortcut in **from your desktop** moves the original file here rather than leaving a duplicate behind; removing that shortcut from DeskRipple puts the original back on your desktop, and right-click → "Restore original to desktop" brings it back anytime. Prefer copies? Switch to "Copy it" under **Settings → Behavior**. |
...
1. **Automatic update checks** — every few hours, DeskRipple asks GitHub whether a newer version exists and, if so, downloads it in the background. Off switch: **Settings → Updates → "Install updates automatically."**
...
- **Turn off automatic updates:** Settings → Updates → "Install updates automatically." Only the single critical-update check at launch remains (see "Network activity").
...
> **Maintainer note — keep this in sync with `TelemetryService` / `TelemetryPing`.** Before pointing `AppInfo.TelemetryEndpoint` at a real host: pick an analytics provider, confirm whether it stores request IP addresses (an IP is personal data under GDPR — disable or anonymize it), prefer EU hosting with a signed DPA, and add the provider's name, location, and a link to its privacy policy here.
