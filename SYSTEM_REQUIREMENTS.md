...
DeskRipple is a Windows desktop folder dock built on .NET 8 + WPF, targeting `win-x64`. The .NET runtime, WPF, and Visual C++ runtime all ship inside the download (the managed runtime in the executable, the native WPF/VC++ DLLs alongside it) — running `DeskRipple-beta-Setup.exe` is the whole install: per-user, no admin prompt, no separate runtime downloads.

## Minimum

| Component   | Requirement                                                                 |
|-------------|------------------------------------------------------------------------------|
| OS          | Windows 11 (any retail build, 64-bit)                                        |
...
| GPU         | Any GPU supported by Windows 11 (DWM composition is required for blur)       |
...
| GPU       | Any dedicated or modern integrated GPU — smoother acrylic blur and animations |
...
- **No native ARM64 build.** DeskRipple will run on Windows on ARM via Microsoft's x64 emulation layer, but blur smoothness and startup time suffer. Native ARM64 is not officially supported.
- **Windows 10 is not officially supported.** The application manifest permits Windows 10, and most features will work on Windows 10 22H2, but desktop icon displacement (via `IFolderView2`) and the acrylic backdrop are tuned and tested against Windows 11. Run on Windows 10 at your own risk.
